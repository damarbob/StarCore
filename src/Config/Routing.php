...
class Routing extends BaseRouting
{
    ...
    {
        /** @var \StarCore\Config\Star */
        $star = config('Star');

        $routesPath = 'Config' . DIRECTORY_SEPARATOR . 'Routes.php';

        // Collect dev modules first
        foreach ($star->getActiveDevModules() as $module) {
            $path = $star->devModulesPath . $module . DIRECTORY_SEPARATOR . $routesPath;
            if (file_exists($path)) $moduleRoutes[] = $path;
        }

        // Then regular modules
        foreach ($star->getActiveModules() as $module) {
            $path = $star->modulesPath . $module . DIRECTORY_SEPARATOR . $routesPath;
            if (file_exists($path)) $moduleRoutes[] = $path;
        }
